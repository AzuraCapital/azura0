...
function Page() {
  ...
  const { data: assets } = useQuery({
    ...
    enabled: !!user,
    queryFn: async () => (await supabase.from("assets").select("*, asset_categories(name), custody_accounts(institution_name)").order("created_at", { ascending: false })).data ?? [],
  });
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="glass rounded-3xl p-6 flex items-center gap-4">
        <div className="rounded-full p-3 bg-primary/10 text-primary"><TrendingUp className="h-6 w-6" /></div>
        <div>
        ...
        {(assets ?? []).map((a: any) => (
          <div key={a.id} className="glass rounded-3xl p-5">
            <div className="flex items-start justify-between">
              <div>
                <div className="text-xs text-primary font-medium uppercase tracking-wide">{a.asset_categories?.name}</div>
            ...
            <div className="mt-4 pt-4 border-t border-border/50 space-y-1 text-sm">
              ...
              {a.custody_accounts && <div className="flex justify-between"><span className="text-muted-foreground">Custódia</span><span>{a.custody_accounts.institution_name}</span></div>}
            </div>
          </div>
        ))}
      </div>
...
function AssetModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  const { user } = useAuth();
  const [form, setForm] = useState({ name: "", ticker: "", asset_category_id: "", quantity: "", avg_price: "", acquisition_date: "", notes: "" });
  ...
  return (
    <Modal open={open} onClose={onClose} title="Novo Ativo">
      <div className="space-y-3 max-h-[70vh] overflow-y-auto pr-1">
        <Field label="Categoria"><SelectInput value={form.asset_category_id} onChange={e => setForm({ ...form, asset_category_id: e.target.value })}>
